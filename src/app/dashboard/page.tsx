...
export default function DashboardPage() {
  ...
  function load(o = owner, st = start, en = end) {
    ...
    fetch('/api/dashboard?' + qs.toString())
      ...
      .catch(e => { setErr('Nu am putut încărca indicatorii (' + (e?.message || 'eroare rețea') + '). Reîncerc automat în 30s.'); });
  }
  ...
  if (!s) return (
    <Layout>
      <div className="card card--pad" style={{ textAlign: 'center', color: 'var(--text-muted)', padding: 40 }}>
        {err
          ? (<><div style={{ color: 'var(--danger)', marginBottom: 12 }}>{err}</div><button className="btn btn-primary" onClick={() => load()}>Reîncearcă acum</button></>)
  ...
  const funnel = [
    { label: 'Intrări CRM', n: fn.intrari, color: 'var(--st-intrare)' },
    { label: 'T1 făcut', n: fn.t1, color: 'var(--st-t1)' },
    { label: 'Nevoie identificată', n: fn.nevoie, color: 'var(--info)' },
    { label: 'Schiță trimisă', n: fn.schita, color: 'var(--st-schita)' },
    { label: 'Pre-Ofertat', n: fn.preofertat, color: 'var(--st-preofertat)' },
    { label: 'Ofertat', n: fn.ofertat, color: 'var(--st-ofertat)' },
    { label: 'Contract semnat', n: fn.contractat, color: 'var(--st-contractat)' },
  ];
  ...
  const conversii: [string, string][] = [
    ['Nevoie / T1', pct(fn.nevoie, fn.t1)],
    ['Schiță / Nevoie', pct(fn.schita, fn.nevoie)],
    ['Pre-Ofertat / Schiță', pct(fn.preofertat, fn.schita)],
    ['Ofertat / Pre-Ofertat', pct(fn.ofertat, fn.preofertat)],
    ['Contract / Ofertat', pct(fn.contractat, fn.ofertat)],
  ];

  // Raport de activitate — cifre reale din stats.
  const raport: [string, number][] = [
    ['Clienți Intrați', fn.intrari],
    ['Clienți Sunați (T1)', fn.t1],
    ['Nevoie acoperită', fn.nevoie],
    ['Contractați', s.byStadiu['Contractat'] ?? 0],
    ['Suprafață totală (m²)', s.totalSuprafata],
  ];
  ...
  const topbar = (
    <div className="topbar__tools" style={{ display: 'flex', alignItems: 'center', gap: 10, flexWrap: 'wrap', marginLeft: 'auto' }}>
      ...
      <label className="d2ctrl" style={{ flexDirection: 'row', alignItems: 'center', gap: 5 }}>
        <span className="muted" style={{ fontSize: '.6875rem' }}>De la</span>
      ...
      <label className="d2ctrl" style={{ flexDirection: 'row', alignItems: 'center', gap: 5 }}>
        <span className="muted" style={{ fontSize: '.6875rem' }}>până la</span>
      ...
      {(start || end) && (
        <button className="btn btn-ghost btn-sm" title="Șterge intervalul" onClick={() => { setStart(''); setEnd(''); }}><Icon name="x" size={15} /></button>
      )}
      {isManager && (
    ...
    <Layout title="Dashboard" topbar={topbar}>
      <div className="dash2">
        ...
        <div className="dash2__bar">
          <h2>Dashboard</h2>
          <span className="dash2__sub">Privire de ansamblu · {intervalLbl}</span>
        </div>

        {/* Cifre cheie */}
        <div className="d2sec-lbl">Cifre cheie</div>
        <div className="d2-keyrow">
          <div className="d2key"><span className="d2key__l">Clienți în pâlnie</span><span className="d2key__v">{num(s.total)}</span><span className="d2key__s">în intervalul selectat</span></div>
          <div className="d2key"><span className="d2key__l">Rată conversie (contract / intrați)</span><span className="d2key__v">{conv}%</span><span className="d2key__s">contract semnat din total</span></div>
          <div className="d2key"><span className="d2key__l">Deal-uri semnate</span><span className="d2key__v">{s.byStadiu['Contractat'] ?? 0}</span><span className="d2key__s">status = Contractat</span></div>
          <div className="d2key"><span className="d2key__l">Suprafață totală (m²)</span><span className="d2key__v accent">{num(s.totalSuprafata)}</span><span className="d2key__s">suma mp clienți în interval</span></div>
        </div>

        {/* Funnel — bare pe etape (7 trepte) */}
        <div className="card card--pad">
          <div className="d2sec-lbl">Cum curge pâlnia</div>
          ...
          <div className="muted" style={{ fontSize: '.6875rem', marginTop: 12 }}>
            T1 = status (s-a făcut primul contact), nu o treaptă de conversie propriu-zisă. „Nevoie identificată" = nevoie acoperită (eventual cu condiții).
          </div>
        </div>
        ...
        <div className="d2-2col">
          <div className="card card--pad">
            <div className="d2sec-lbl">Conversii între etape</div>
          ...
          <div className="card card--pad">
            <div className="d2sec-lbl">Raport activitate</div>
            <div className="d2list">
        ...
        <div className="d2-2col">
          <div className="card card--pad">
            <div className="d2sec-lbl">Distribuție pe stadiu</div>
            <div className="d2funnel">
              {stadiuEntries.map(([k, v]) => {
                ...
                return (
                  <div key={k} className="d2fn" style={{ cursor: 'default', gridTemplateColumns: '150px 36px 1fr 48px' }}>
                    <span className="d2fn__lbl">{k || 'în lucru'}</span>
          ...
          <div className="card card--pad">
            <div className="d2sec-lbl">Distribuție pe prioritate</div>
            <div className="d2list">
              {[4, 3, 2, 1, 0].map(lvl => {
                ...
                return (
                  <div key={lvl} className="d2fn" style={{ cursor: 'default', gridTemplateColumns: '78px 36px 1fr 48px' }}>
                    <span className="d2fn__lbl" style={{ display: 'inline-flex', alignItems: 'center' }}>
                      {lvl === 0 ? <span className="muted" style={{ fontSize: '.6875rem' }}>fără stea</span> : <PriorityStars value={lvl} readOnly size={14} />}
                    </span>
                    <span className="d2fn__n mono">{v}</span>
        ...
        <div className="d2-2col">
          <div className="card card--pad">
            <div className="d2sec-lbl">Pe categorie</div>
            <div className="d2-keyrow" style={{ gridTemplateColumns: 'repeat(auto-fit, minmax(96px, 1fr))' }}>
              {Object.entries(s.byCategorie).sort((a, b) => Number(a[0]) - Number(b[0])).map(([k, v]) => (
                <div key={k} className="d2key">
                  <span className="d2key__l">{catLabels[k] || 'Cat ' + k}</span>
          ...
          <div className="card card--pad">
            <div className="d2sec-lbl">Sincronizări recente</div>
            {s.recentSyncs.length === 0 && <div className="muted" style={{ fontSize: '.8125rem' }}>Nicio sincronizare încă.</div>}
            <div className="d2list">
