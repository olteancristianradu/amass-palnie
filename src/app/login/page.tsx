      ...
      <div className="sidebar hidden md:flex flex-col justify-between w-[42%] p-12 text-white">
        ...
        <div>
          <div className="font-display text-[40px] leading-[1.05] font-semibold text-white max-w-sm">
            {t('Pâlnia ta de clienți, conectată la CRM.')}
          </div>
          <p className="text-[var(--on-ink-soft)] mt-4 max-w-sm text-sm leading-relaxed">
            {t('Sincronizezi clienții, completezi fișa de strategie cu calcul automat și împingi totul în gestcom — dintr-un singur loc.')}
          </p>
        ...
        <div className="text-[11px] uppercase tracking-[.22em] text-[var(--on-ink-soft)]">{t('Pâlnie Clienți')}</div>
      </div>

      {/* Form */}
      <div className="flex-1 flex items-center justify-center p-8">
        <div className="w-full max-w-sm rise">
          <h1 className="font-display text-2xl mb-1">{t('Bine ai revenit')}</h1>
          <p className="text-[var(--fg-soft)] text-sm mb-7">{t('Loghează-te cu emailul de firmă.')}</p>
          <form onSubmit={onSubmit} className="space-y-4">
            <div>
              <label className="kpi-label block mb-1.5">{t('Email firmă')}</label>
              <input className="field" type="email" value={email} onChange={e => setEmail(e.target.value)} required autoFocus />
            </div>
            <div>
              <label className="kpi-label block mb-1.5">{t('Parolă')}</label>
              <input className="field" type="password" value={password} onChange={e => setPassword(e.target.value)} required />
            </div>
            {error && <div className="toast toast-err">{t(error)}</div>}
            <button type="submit" disabled={loading} className="btn btn-primary w-full justify-center py-2.5">
              {loading ? t('Se autentifică…') : t('Intră în consolă')}
            </button>
          </form>
        </div>
      </div>
