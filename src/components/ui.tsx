...
export function PriorityStars({ value, onSet, size = 16, readOnly = false }: {
  ...
  if (readOnly || !onSet) return <span title={STEA_TITLE[value] || ''} style={{ lineHeight: 0 }}>{oneStar(value, size)}</span>;
  // Editabil: selector cu 5 culori (fără / roșu / portocaliu / albastru / verde) — click pe culoarea dorită.
  return (
    <div className="inline-flex items-center gap-[3px]" onClick={e => e.stopPropagation()} title="Setează culoarea steluței (ca în CRM)">
      {[0, 1, 2, 3, 4].map(c => (
        <button key={c} type="button" title={STEA_TITLE[c]} onClick={() => onSet(c)}
...
export function SyncBadge({ last, syncing, auto }: { last?: SyncInfo | null; syncing?: boolean; auto?: AutoSyncInfo | null }) {
  if (syncing || auto?.inFlight) {
    return (
      <span className="sync-badge" title="Sincronizare în curs cu CRM">
  ...
  if (auto?.enabled) {
    if (auto.lastError) {
      return (
        <span className="sync-badge" title={'Eroare auto-sync: ' + auto.lastError + ' (reîncearcă automat)'}>
          <span className="sync-dot" style={{ background: 'var(--sync-old)' }} />
          Auto-sync: eroare, reîncerc…
        </span>
      );
    }
    ...
    if (!iso) {
      return (
        <span className="sync-badge" title="Auto-sync pornit — prima verificare în curând">
    ...
    return (
      <span className="sync-badge" title={`Auto-sync activ: verificare la ~${auto.lightEverySec ?? 90}s, detalii la ~${Math.round((auto.detailEverySec ?? 600) / 60)}min`}>
        <span className="sync-dot live" style={{ background: color }} />
        Auto • {agoText(ageMin, iso)}
      </span>
    );
  }
  ...
  if (!last || !last.startedAt) {
    return (
      <span className="sync-badge" title="Nicio sincronizare încă"><span className="sync-dot" style={{ background: 'var(--fg-faint)' }} />Nesincronizat</span>
    );
  }
  ...
  return (
    <span className="sync-badge" title={`Ultimul sync: ${last.type ?? ''} · ${last.processed ?? 0}/${last.total ?? 0} · ${new Date(last.startedAt).toLocaleString('ro-RO')}`}>
      <span className="sync-dot" style={{ background: color }} />
      {failed ? 'Sync eșuat' : `Sincronizat ${agoText(ageMin, last.startedAt)}`}
